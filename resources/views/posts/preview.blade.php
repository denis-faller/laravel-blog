<div class="col-lg-4 mb-4">
    <div class="entry2">
      <a href="{{route('post.show', $post->url)}}"><img src="{{$post->preview_img}}" alt="Image" class="img-fluid rounded"></a>
      <div class="excerpt">
      @foreach($post->tags as $tag)    
      <span class="post-category text-white mb-3" style = "background-color:{{$tag->color}}">{{$tag->name}}</span>
      @endforeach
      <h2><a href="{{route('post.show', $post->url)}}">{{$post->name}}</a></h2>
      <div class="post-meta align-items-center text-left clearfix">
        ...
        <p><a href="{{route('post.show', $post->url)}}">Read More</a></p>
      </div>
    </div>
</div>